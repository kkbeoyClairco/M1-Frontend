...
import { getPM25Color, getPM10Color } from 'utils/AQI/colorUtils';
const PMChart = ({ data, lastUpdated, infoClickFn }: any) => {
    const [showText, setShowText] = useState(true);
    let Data =
        data && Object.keys(data).length > 0
            ? [roundToZeroDecimal(data?.pm10), roundToZeroDecimal(data?.pm25)]
            : [0, 0];
    ...
    if (data?.pm10 === 0) series.pop();
    if (data?.pm25 === 0) series.shift();

    useEffect(
        function swithData() {
            let interval: NodeJS.Timeout;
            if (data && data.pm10 && data.pm25) {
                interval = setInterval(() => {
                    setShowText((prev) => !prev);
                }, 3000); // Change every 3 seconds
            } else if (data && data.pm25) {
                setShowText(true);
            } else if (data && data.pm10) {
                setShowText(false);
            }
        ...
        <Card style={{ width: '100%', color: 'black' }}>
            ...
            <Card.Body style={{ padding: '0' }}>
                <div style={{ display: 'flex', padding: '0px', justifyContent: 'center', alignItems: 'center' }}>
                    <h5 style={{ padding: '10px', paddingLeft: '15px' }}>Indoor Particulate Matter</h5>{' '}
