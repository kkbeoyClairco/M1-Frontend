...
import { coloursTable, unitTables } from 'appConstants/propertyTable';
// import {
...
import { timeGrouingConstants } from 'appConstants/trendsConstants';
import { deviceStatusConstant } from 'appConstants/claircoConstants';
...
interface TrendsChartInterface {
    deviceAliasName?: string;
    deviceName: string;
}

const TrendsChart: React.FC<TrendsChartInterface> = ({ deviceAliasName, deviceName }) => {
    ...
    const getTrendData = useCallback(
        async (sensorName: string) => {
            try {
                ...
                const res: any = (await getIndoorUnitLiveData(deviceTypeId1, sensorName, timeGroup)) ?? {};
    ...
    useEffect(() => {
        if (graphState === graphOptions.a) getTrendData(deviceName);
        else if (graphState === graphOptions.b) getAggregateData();
