...
const MultipleQuadrantPlotting: React.FC<QuadrantPlottingInterface> = ({
    imageURL,
    changeZonetFn,
    scalingFactor,
    selectedQuadrant,
    ...
    // const [zones, setZones] = useState<{ name: string; points: PointInterface[] }[]>([]);
    const [draggingZoneIndex, setDraggingZoneIndex] = useState<number | null>(null);
    const [draggingPoint, setDraggingPoint] = useState<{ zone: number; point: number } | null>(null);
    ...
    const handleClick = (event: { target: { getStage: () => any }; evt: any }) => {
        ...
        const newPoints = [...points, { x: pointerPosition.x, y: pointerPosition.y }];
        if (newPoints.length === 4) {
            const zoneIndex = zonesList?.length;
            const newZone = [...zonesList, { name: `Quadrant ${zoneIndex + 1}`, points: newPoints }];
            // setZones(newZone);
    ...
    // const handleQuadEdgeDragMove = (
    //     zoneIndex: number,
    //     index: number,
    //     e: { target: { x: () => any; y: () => any } }
    // ) => {
    //     console.log('Edge point Dragging');
    //     if (!e.target) return;
    //     // const newPoints = [...points];
    //     const newX = e?.target?.x();
    //     const newY = e?.target?.y();
    ...
    //         console.log('Out of bounds, reverting to previous position');
    //         const target = e.target as Konva.Node; // Replace Konva.Node with the correct type
    //         target.x(zones?.[zoneIndex]?.points?.[index]?.x);
    //         target.y(zones?.[zoneIndex]?.points?.[index]?.y);
    //         // e.target.x(points[index].x); // Reset x position
    //         // e.target.y(points[index].y); // Reset y position
    //         return;
    //     }
    ...
    //     // if (newPoints[index].x > 400 || newPoints[index].x<0> || newPoints[index].y) return;
    //     // console.log(newPoints[index]);
    //     const newZonesData = zones.map((zone, zIdx) => {
    //         if (zIdx === zoneIndex) {
    //             const updatedPoints = zone.points.map((pt, ptIdx) => (ptIdx === index ? { x: newX, y: newY } : pt));
    ...
    const handleQuadEdgeDragMove = (
        ...
        if (
            ...
            target.x(zonesList?.[zoneIndex]?.points?.[index]?.x);
            target.y(zonesList?.[zoneIndex]?.points?.[index]?.y);
            return;
        }

        // Only update the zone's points
        const newZonesData = zonesList.map((zone, zIdx) => {
            if (zIdx === zoneIndex) {
                const updatedPoints = zone.points.map((pt, ptIdx) => (ptIdx === index ? { x: newX, y: newY } : pt));
                return { ...zone, points: updatedPoints };
            }
        ...
        changeZonetFn(newZonesData);
    };
    // const handleZoneDragMove = async (zoneIndex: number, e: any) => {
    ...
    //             y: pt.y + y,
    //         }));
    //         return { ...zone, points: movedPoints };
    //     });
    //     setZones(newZones);
    ...
    //     let updatedPoints = [...zone.points];

    //     if (edge === 'right') {
    //         // Move top-right and bottom-right x to pointer.x
    //         updatedPoints[1] = { ...updatedPoints[1], x: pointer.x };
    //         updatedPoints[2] = { ...updatedPoints[2], x: pointer.x };
    //     } else if (edge === 'left') {
    //         // Move top-left and bottom-left x to pointer.x
    //         updatedPoints[0] = { ...updatedPoints[0], x: pointer.x };
    //         updatedPoints[3] = { ...updatedPoints[3], x: pointer.x };
    //     } else if (edge === 'top') {
    //         // Move top-left and top-right y to pointer.y
    //         updatedPoints[0] = { ...updatedPoints[0], y: pointer.y };
    //         updatedPoints[1] = { ...updatedPoints[1], y: pointer.y };
    //     } else if (edge === 'bottom') {
    //         // Move bottom-left and bottom-right y to pointer.y
    //         updatedPoints[2] = { ...updatedPoints[2], y: pointer.y };
    //         updatedPoints[3] = { ...updatedPoints[3], y: pointer.y };
    //     }
                    ...
                    <Layer>
                        ...
                        {zonesList?.map((zone, i) => (
                            <Group
                                key={i}
                                // onDragStart={(e) => {
                                //     setDraggingZoneIndex(i);
                                //     e.cancelBubble = true;
                                // }}
                                draggable={draggingEdge === null}
...
export default MultipleQuadrantPlotting;
