...
export const OccupantsLayoutSVG = ({
    ...
    const handleClick = async (element: any) => {
        try {
            ...
            //     const url = `/customer/occupancy/${occupancySensorName}`;
    ...
    const handleHover = throttle((e: any) => {
        try {
            ...
            if (!id) return;
            const idArray = toolTipData1?.map((doc: any) => doc.id);
            ...
            try {
                fetch(
                    ...
                    .then(async (svgContent) => {
                        ...
                        const svgElement = svgDoc?.getElementById('clairco-svg') as HTMLElement;
