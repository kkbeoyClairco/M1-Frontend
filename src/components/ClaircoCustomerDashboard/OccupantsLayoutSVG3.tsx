...
import { LayoutToolTip2 } from './ToolTip/LayoutToolTip2';

const toolTipData: { [key: string]: string | number } = {
    count: '57',
    cubical1: 0,
    cubical2: 1,
    cubical3: 1,
...
export const OccupantsLayoutSVG2 = ({
    floorPlanUrl,
    currentState,
    elementsToGreen,
    elementsToRed,
    ...
    const handleClick = async (element: any) => {
        try {
            ...
            let url = '';
            if (id === 'odour')
                url = `odour/name=IAQ_GRR1&building=BAN-04&location=Bangalore&floor=+Floor+5&zone=Unit%C2%A01%C2%A0GRR`;
            else if (id === 'count')
                url = `sensiCount/name=PCS_GRR1&building=+BAN-04&location=Bangalore&floor=Floor+5&zone=Unit%C2%A01%C2%A0GRR`;
            else if (id === 'feedback') url = `feedback/name=Unit+1+GRR&building=BAN-04&floor=Floor+5&customer=Clairco`;
            else return;
            navigate(url);
        } catch (error) {
            console.log(error);
    ...
    const handleHover = throttle((e: any) => {
        try {
            ...
            if (!id) return;
            const value = Object.keys(toolTipData).filter((key) => key === id)?.[0];
            ...
            // });
            let type = id;
            if (id.startsWith('cubical')) type = 'cubical';
            const dataToToolTip = {
                type,
