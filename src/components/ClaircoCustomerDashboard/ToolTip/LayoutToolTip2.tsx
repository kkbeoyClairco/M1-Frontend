...
const OdourComponent: React.FC<any> = ({ data }) => {
    return (
        <div style={{ display: 'grid', gap: '0', justifyContent: 'center' }}>
            {data && (
                <>
                    {' '}
                    <Row style={{ paddingInline: '10px', width: '220px' }}>
                        <Col xs={6} style={{ padding: '0px', textAlign: 'start' }}>
                            Odour Level:{' '}
                        </Col>
...
// Component For Occupants State
const FeedbackComponent: React.FC<any> = ({ data }) => {
    return (
        <div style={{ display: 'grid', gap: '0', justifyContent: 'center' }}>
            {data && (
                <>
...
// Component For Occupants State
const CubicalComponent: React.FC<any> = ({ data }) => {
    return (
        <div style={{ display: 'grid', gap: '0', justifyContent: 'center' }}>
            {data && (
                <>
                    ...
                    <Row style={{ paddingInline: '10px', width: '220px' }}>
                        ...
                        <Col xs={6}>{data.value === 1 ? 'Yes' : 'No'} </Col>
                    </Row>
                    <Row style={{ paddingInline: '10px', width: '220px' }}>
                        <Col xs={6} style={{ padding: '0px', textAlign: 'start' }}>
                            Updated on:{' '}
...
// Component For Occupants State
const OccupancyDataComponent: React.FC<any> = ({ data }) => {
    return (
        <div style={{ display: 'grid', gap: '0', justifyContent: 'center' }}>
            {data && (
                <>
...
export const LayoutToolTip2 = ({ show, positionValues, toolTipData, currentState }: any) => {
    // console.log('Tool tip data', toolTipData);
    const [toolTipPosition, setToolTipPosition] = useState({});
    const getComponent = (type: string, data: any) => {
        switch (type) {
            case 'count':
                return <OccupancyDataComponent data={data} />;

            case 'cubical':
                return <CubicalComponent data={data} />;

            case 'feedback':
                return <FeedbackComponent data={data} />;

            case 'odour':
                return <OdourComponent data={data} />;
        }
    };
    ...
    return (
        <div
            ...
            {toolTipData?.type && <h5 style={{ textAlign: 'center' }}>{getHeading(toolTipData?.type)}</h5>}
            {toolTipData?.type && getComponent(toolTipData?.type, toolTipData)}
