...
const OccupantsLayout = ({ floorId, devices, floorPlanUrl }: LayoutPropsType) => {
    ...
    const getDataForLayout = useCallback(async (floorId: string, currentState: string) => {
        try {
            ...
            if (currentState === graphOptions['VRF-VRV']) {
                ...
                setGreenAHU(greenElementsAHU);
                setRedAHU(redElementsAHU);
                const ahuDataToolTip = await getAhuToolTipData(data?.data?.devices);
                // console.log('ahu', ahuDataToolTip);
                setToolTipData(ahuDataToolTip);
                ...
                setToolTipData(occupantsDataToTooltip);
            }
    ...
    useEffect(() => {
        if (floorId) getDataForLayout(floorId, graphState);
    }, [graphState, floorId]);
        ...
        <Col md={12} style={{ padding: '20px', paddingTop: '10px' }}>
            <Card style={{ height: '665px' }}>
                ...
                <Row style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '20px' }}>
                    <Col md={6} style={{ padding: '20px' }}>
                        ...
                        <Row style={{ paddingLeft: '20px' }}>
                            <h6>
                                {data?.name} &gt;{buildingName} &gt; Floor {floorName}
                            </h6>
                        </Row>
                    </Col>
                                            ...
                                            <div
                                                ...
                                                <Nav.Link
                                                    ...
                                                    style={{
                                                        ...
                                                        textAlign: 'center',
                                                        fontWeight:
                                                            graphState === graphOptions[option] ? 'bold' : '100',
                                                    }}
                                                    className="py-1"
                                                    eventKey="device"
                ...
                <Row style={{ height: '534px' }}>
                    {' '}
                    {activeDevices.length !== 0 && floorId && floorPlanUrl && (
                        <OccupantsLayoutSVG
