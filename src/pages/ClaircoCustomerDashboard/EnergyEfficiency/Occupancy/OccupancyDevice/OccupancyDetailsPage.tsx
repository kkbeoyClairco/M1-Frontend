...
import { occupancyTrendsData } from 'appConstants/dataToSvg';
// import empty from 'assets/images/empty';
// import occupied from 'assets/images/occupied';
const empty = `https://res.cloudinary.com/dlulq6hny/image/upload/v1751459446/empty_hcz29h.jpg`;
const occupied = `https://res.cloudinary.com/dlulq6hny/image/upload/v1751459446/occupied_o3xch0.jpg`;
...
const OccupancyDetailsPage = () => {
    ...
    const [graphXAxis, setGraphXAxis] = useState<any>([]);
    ...
    const getOccupantsData = useCallback(
        async (deviceId, timePeriod) => {
            try {
                ...
                const data: any = occupancyTrendsData;
                //  await getRawOccupancyData(
                ...
                // console.log('Data for occupa:', thermopileId[thermopileId.length - 1], thermopileId);
    ...
    const handleDatePick = async () => {
        try {
            ...
            const endEpochTime = convertDateToEpoch(endDate);
            const res = occupancyTrendsData;
            // await getOccupancywithDates(
            //     Math.floor(startEpochTime / 1000),
            //     Math.floor(endEpochTime / 1000),
            //     deviceId
            // );
            const occupancyNumberArray = res?.map((doc: any) => {
                return doc.metaData['occupancy_number'];
            });
            const extractTime = res?.map((index: any) => {
                const extractedTime = index['epochTime'];
                const time = convertUnixToIST(extractedTime);
                return time;
            });
            const thermopileId = res?.map((doc: any) => {
                return doc.rawDataId;
            });
            getThermalImage(thermopileId[thermopileId.length - 1]);
    ...
    const getThermalImage = async (id: any) => {
        try {
            ...
            setThermalImage((prev) => (prev === empty ? occupied : empty));
