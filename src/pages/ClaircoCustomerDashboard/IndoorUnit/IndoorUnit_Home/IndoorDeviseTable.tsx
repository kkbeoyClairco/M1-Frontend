...
const IndoorDevicesTable = ({ setTotalDevices }: any) => {
    ...
    const handleNavigation = (data: any) => {
        ...
        const { buildingId, locationId, floorId } = data || {};
        const name = data?.name || '';
        const buildingName = buildingId?.name ? buildingId?.name : '';
        const locationName = locationId?.name ? locationId?.name : '';
    ...
    const getIndoorDeviceList = async () => {
        try {
            setIsLoading(true);

            const deviceId = deviceTypeId['VRV/VRF'];
            const response = await fetchDevicesList(
                deviceId,
                // '66aca68db180b45b2dfe62a3'
                customerId
            );
            // const { data } = response;
            if (response?.data?.length === 0) setIsEmpty(true);
            // console.log('Indoor', response);
        ...
        <Row style={{ paddingRight: '0px' }}>
            ...
            <Card>
                <Card.Body>
                    <Row>
                        <Col>
                            <h4 className="header-title mb-3">Indoor Device List</h4>
                        </Col>
                    </Row>
                    {!devicesExists && (
                        <Row>
                            <Col>
                                <h4 className="header-title mb-3">No Indoor devices installed here</h4>
                            </Col>
                        </Row>
                    )}
                    {!isLoading ? (
...
export default IndoorDevicesTable;
