...
import { pcsTrends } from 'appConstants/pcs';
const TrendsChart = ({ sensorName }: any) => {
    ...
    const processResponseData = useCallback((res: any) => {
        const occupancyArray = res?.data?.reverse().map((doc: any) => doc?.inCount);
        const xAxisArray = res?.data?.map((doc: any) => convertUnixToIST(doc?.['timestamp']));
        console.log('Agg Triggered', occupancyArray, res);
        ...
        try {
            ...
            const fetchData = async () => {
                if (graphState === graphOptions.b) {
                    ...
                    const res = { data: pcsTrends }; // await getPcsData(sensorName, startEpochTime.toString(), endEpochTime.toString());
                    processResponseData(res);
                }
            };
    ...
    useEffect(
        function apiCallHandler() {
            ...
            else if (graphState === graphOptions.a) getLiveDataAPI('', '');
        },
