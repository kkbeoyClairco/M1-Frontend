...
import { EnergyMeterTriParameterWidget } from 'components/ClaircoCustomerDashboard/Widgets/EnergyMeterTriParameterWidget';
...
const EnergyMeterPage = () => {
    ...
    const fetchPowerHistoricData = useCallback(async () => {
        try {
            if (!sensorName) return;
            const result: any = await getPowerHistoricData(sensorName, powerTimeState, timeframe2);
    ...
    }, [sensorName, powerTimeState, timeframe2]);
    // Gets PF & Frequency Historic Data
    const fetchPfHistoricData = useCallback(async () => {
        try {
            if (!sensorName) return;
            const result: any = await getEnergyMeterHistoricReadings(sensorName, pfTimeState);
    ...
    }, [sensorName, pfTimeState]);
    // Gets Voltage Historic Data
    const fetchVoltageHistoricData = useCallback(async () => {
        try {
            if (!sensorName) return;
            const result: any = await getEnergyMeterHistoricReadings(sensorName, voltageTimeState);
    ...
    }, [sensorName, voltageTimeState]);
    // Gets Current Historic Data
    const fetchCurrentHistoricData = useCallback(async () => {
        try {
            if (!sensorName) return;

            const result: any = await getEnergyMeterHistoricReadings(sensorName, currentTimeState);
    ...
    }, [sensorName, currentTimeState]);
    // Gets Live Data
    const fetchLiveData = useCallback(
        async (sensorName: any) => {
            try {
                if (!sensorName) return;

                const response: any = await getEnergyMeterLiveReading(sensorName);
                if (response.data[0]) {
    ...
    return (
        <>
            {' '}
            {/* <EnergyMeterTriParameterWidget title={''} multiParamsArray={[]} unit={''} updatedTime={''} /> */}
            <Row>
